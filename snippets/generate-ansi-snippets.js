...
const rust = runBat("rust.rs");
const vitest = runVitest();
const screenfetch = runScreenfetch();
const colortest = execSync("./colortest.sh", { encoding: "utf8" });
...
const rustHtml = au.ansi_to_html(rust).trim();
const colortestHtml = au.ansi_to_html(colortest); // no 'trim()' here as colortest relies on leading whitespace
...
export const rustSnippet = \`${rustHtml}\`;
export const colortestSnippet = \`${colortestHtml}\`;
`;
